...
inline void Key::addOne(){
	
}

inline word_type Key::getMdigit() {
	return m_digit;
}
...
Key Key::subtract(Key top, Key bottom, string one) {
    int t;
    int carry = 0;
	...
	for (int c = C-1; c >= 0; --c) {
		for (int f = 0; f < 5; f++) {
			bottom.m_digit[c] ^= 1 << f;
		}
	}

	//cout << "bottom flipped: " << bottom.toString() << endl;
	//add 1 to bottom
	
	bottom += one;

	//cout << "bottom flipped + 1: " << bottom.toString() << endl;

	//add top to negated bottom
	carry = 0;
    for (int i = C-1; i >= 0; --i) {
       t = top.m_digit[i];	
       top.m_digit[i] = (t + bottom.m_digit[i] + carry)  % R; 
       carry      = (t + bottom.m_digit[i] + carry) >= R; 
    }
...
inline string Key::toString() {
	s = "";
	for_each(m_digit.begin(), m_digit.end(), [](const char& c) { 
		s = s + ALPHABET[c];
		//alternate(ALPHABET[c]);
	});
	return s;
}

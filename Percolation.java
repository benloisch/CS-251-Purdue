...
public class Percolation {
    ...
    public void open(int x, int y) {
        ...
        if (nextTo(x, y, x-1, y))
            quuf.union(x + (rowSize * y), (x-1) + (rowSize * y));
        if (nextTo(x, y, x+1, y))
            quuf.union(x + (rowSize * y), (x+1) + (rowSize * y));
        if (nextTo(x, y, x, y-1))
            quuf.union(x + (rowSize * y), (x) + (rowSize * (y-1)));
        if (nextTo(x, y, x, y+1))
            quuf.union(x + (rowSize * y), (x) + (rowSize * (y+1)));
        ...
        if (isFull(x, y)) {
            ...
            for (int xGrid = 0; xGrid < rowSize; xGrid++)
                for (int yGrid = 0; yGrid < rowSize; yGrid++) {
                    if (quuf.connected(x + (rowSize * y), xGrid + (rowSize * yGrid))) {
    ...
    public boolean isFull(int x, int y) {
        ...
        if (quuf.connected(x + (rowSize * y), rowSize * rowSize))
            return true;

        //for (int xGrid = 0; xGrid < rowSize; xGrid++)
        //    if (quuf.connected(x + (rowSize * y), xGrid + (rowSize * (rowSize-1))))
        //        return true;
    ...
    public boolean percolates() {
        ...
        if (quuf.connected(0, rowSize * rowSize))
            return true;

        //for (int x = 0; x < rowSize; x++)
        //    for (int xGrid = 0; xGrid < rowSize; xGrid++)
        //        if (quuf.connected(x, xGrid + (rowSize * (rowSize-1))))
        //            return true;
        return false;
    }
